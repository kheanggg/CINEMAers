...
export default function SeatSelection({
    ...
    const { movieTitle, time, date, format, hall, cinema } = bookingDetails;
    ...
    useEffect(() => {
        ...
        const handleOutsideClick = (event: MouseEvent) => {
            ...
            if (
                ...
                (!confirmationModal || !confirmationModal.contains(event.target as Node))
            ) {
                                ...
                                <React.Fragment key={rowIndex}>
                                    ...
                                    {Array.from({ length: columns }, (_, colIndex) => {
                                        ...
                                        return (
                                            <div
                                                key={seatKey}
                                                onClick={() => toggleSeatSelection(labels[rowIndex], colIndex + 1)}
                                                className={`flex justify-center items-center cursor-pointer transition-all rounded-[50%] h-6 w-6`}
                                            >
                                                <ChairIcon className={`text-center ${isSelected ? "text-[#FF0000]" : "text-[#474343]"}`} />
                    ...
                    <div className="px-10 py-10 bg-[#1D1B1B] rounded-xl border border-white border-opacity-50">
                        <div>
                            ...
                            <div className="mt-9 w-full h-[110px] grid grid-cols-2">
                                ...
                                <h5 className="text-right text-lg">{Object.keys(selectedSeats).length} selected</h5>
                            </div>
                            ...
                            <div className="mt-7 bg-[#474343] w-full h-[50px] py-3 rounded-xl">
                                <div className="grid grid-cols-2 mx-5">
                                    ...
                                    <h5 className="text-[18px] text-[#FF0000] text-right">
                                        {Object.keys(selectedSeats).length * 3.5}$ {/* Example price */}
                                    </h5>
                                </div>
                            </div>
                            <div className="mt-3 bg-[#FF0000] w-[50%] h-[50px] py-3 rounded-2xl mx-auto transition-all duration-300 hover:bg-[#CC0000] active:scale-95 cursor-pointer"
                                onClick={() => { onClose(); setShowConfirmBooking(true); setSelectedSeats2(selectedSeats) }}>
